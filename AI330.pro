...
входит_в(Персонаж,[_|Хвост]):-входит_в(Персонаж,Хвост).

один_из(Элемент,[Элемент|_]).
один_из(Элемент,[Первый|Хвост]):- not(Элемент = Первый),
...
%        2. Б забрасывал и вытаскивал сеть.
    ...
    сказал("А", 1, Поймано_рыбы,_):- Поймано_рыбы = мало.    
    сказал("А", 2, _, Рыбаки):- входит_в(рыбак("Б", _, занимался_сетью, _), Рыбаки).
    
%     Б: 1. Я управлял лодкой, а не занимался сетью.
%        2. Мы наловили много рыбы.
%        3. Если бы В уговорили ответить, он бы три раза повторил, что он сорореанец.
    
    сказал("Б", 1, _, Рыбаки):- входит_в(рыбак("Б", _, управлял_лодкой, _), Рыбаки).    
    сказал("Б", 2, Поймано_рыбы, _):- Поймано_рыбы = много.    
    сказал("Б", 3, _, Рыбаки):- повторил_бы("В", Рыбаки).

    % он бы три раза повторил, что он сорореанец
    повторил_бы(Имя, Рыбаки):- входит_в(рыбак(Имя, сорореанец, _, _), Рыбаки),
    ...
    найти_по_номеру(Номер, Элемент, [Элемент|_]):- Номер - 1 = 0.
    ...
    проверка_возможного_высказывания("В", Рыбаки):- повторил_бы("В", Рыбаки),
    ...
    проверка_племени(Имя, Рыбаки):- входит_в(рыбак(Имя, сорореанец, _, Список_истинности), Рыбаки),
                                    not(один_из(ложно, Список_истинности)).

    % если норореанец, то нет истинных высказываний
    проверка_племени(Имя, Рыбаки):- входит_в(рыбак(Имя, норореанец, _, Список_истинности), Рыбаки),
                                    not(один_из(истинно, Список_истинности)).

    % если мидореанец, то список истинности соответствует ранее определенным
    проверка_племени(Имя, Рыбаки):- входит_в(рыбак(Имя, мидрореанец, _, Список_истинности), Рыбаки),
 ...
 ищем_решение(Рыбаки):-
        ...
        %    Формируем гипотезу
        Рыбаки = [рыбак("А", Племя_А, Роль_А, Список_истинности_А),
