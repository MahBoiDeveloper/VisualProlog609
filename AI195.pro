...
CLAUSES	 		
	...
	входит_в(Подозреваемый,[_|Хвост_Списка_Подозреваемых]):-входит_в(Подозреваемый,Хвост_Списка_Подозреваемых).
	...
	генерация_гипотезы(Подозреваемые):- 
		возможная_вина(ВинаА), возможная_вина(ВинаВ),возможная_вина(ВинаС),
% 1. Каждый из тройки подозреваемых А, В и С в день ограбления побывал в лавке, и никто больше в тот день в лавку не заходил;
		  Подозреваемые=[подозреваемый(а,ВинаА), 
...
% 2. Если А виновен, то у него был ровно один сообщник;				   
	утверждение(2,Подозреваемые):- 
		...
		findall(Имя, входит_в(подозреваемый(Имя, виновен), Подозреваемые), Виновные), 
		количество(Виновные,ЧислоВиновных), ЧислоВиновных=2;
...
% 4. Если виновны ровно двое подозреваемых, то А - один из них;
	утверждение(4,Подозреваемые):- 
		findall(Имя, входит_в(подозреваемый(Имя, виновен), Подозреваемые), Виновные), 
	        количество(Виновные,КоличествоВиновных), КоличествоВиновных=2,
	        входит_в(подозреваемый(а,виновен),Подозреваемые);
	        findall(Имя, входит_в(подозреваемый(Имя, виновен), Подозреваемые), Виновные), 
	        количество(Виновные,КоличествоВиновных), not(КоличествоВиновных=2).
...
% 5. Если С не виновен, то В также не виновен.
	...
	поиск_решения(Имя):-
        ...
        поиск_обвиняемого(РазныеРешения):-
         	findall(ИмяОбвиняемого, поиск_решения(ИмяОбвиняемого), ВсеРешения),   
