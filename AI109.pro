...
CLAUSES
    ...
    убрать_повторы([], Накопитель, СписокБезПовторов):- СписокБезПовторов = Накопитель.
    ...
    генерация_виновностей(Флаг,[не_виновен|Остальные]):-генерация_виновностей(Флаг,Остальные).
    генерация_виновностей(1,[виновен|Остальные]):-генерация_виновностей(0,Остальные).
 	
    генерация_гипотезы(Гипотеза):-
    /*  Генерация гипотезы    */
        возможная_правдивость(Правдивость2),возможная_правдивость(Правдивость3),
        возможное_имя(Обвиняемый1), возможное_имя(Обвиняемый2), возможное_имя(Обвиняемый3),
        ...
        findall(Имя, входит_в(подсудимый(Имя, виновен,_,_), Гипотеза), Виновные), 
	количество(Виновные,КоличествоВиновных), КоличествоВиновных>0.
    ...
    поиск_виновных(ИменаВиновных, Правдивость3):-
     	...
     	findall(Имя, входит_в(подсудимый(Имя, виновен, _,_), Гипотеза), ИменаВиновных).
    ...
    установить_обвиняемого3(Обвиняемый):-
    	findall(ИменаВиновных, поиск_виновных2(ИменаВиновных,первый), ВсеВиновные),
    	убрать_повторы(ВсеВиновные, [], Виновные),
        количество(Виновные,ЧислоРешений), ЧислоРешений=1, Обвиняемый=первый;
    	findall(ИменаВиновных, поиск_виновных2(ИменаВиновных,второй), ВсеВиновные),
    	убрать_повторы(ВсеВиновные, [], Виновные),
        количество(Виновные,ЧислоРешений), ЧислоРешений=1, Обвиняемый=второй.	
    ...
    является_виновным(Имя):-
    	 findall(ИменаВиновных, проверка_гипотезы(ИменаВиновных), ВсеВиновные),
