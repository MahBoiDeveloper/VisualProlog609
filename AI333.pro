...
возможный_список_правдивости_высказываний(Список) :- возможная_правдивость(Правдивость_1),
...
содержит_правдивость_под_номером([Правдивость|_], Правдивость, Номер) :- Номер - 1 = 0.
содержит_правдивость_под_номером([_|Хвост_списка_правдивостей], Правдивость, Номер) :- Номер_1 = Номер - 1,
                                                содержит_правдивость_под_номером(Хвост_списка_правдивостей, Правдивость, Номер_1).  

% А:
%Это сделал Б.
сказал(1, "А", Гипотеза, _) :- входит_в(подозреваемый("Б", виновен, _), Гипотеза).
%Я несколько раз встречался с В.
сказал(2, "А", _, знакомы).               
%У меня полно своих шипослив.
сказал(3, "А", Гипотеза, _) :- входит_в(подозреваемый("А", невиновен, _), Гипотеза).
        
% Б:
%А и В незнакомы и никогда не встречались.
сказал(1, "Б", _, незнакомы).                                     
%Это сделал я.
сказал(2, "Б", Гипотеза, _) :- входит_в(подозреваемый("Б", виновен, _), Гипотеза).
%Третье высказывание А – ложь.
сказал(3, "Б", Гипотеза, Знакомство) :- not(сказал(3, "А", Гипотеза, Знакомство)).
        
% В:
%Я незнакомы с А и никогда его не видел.
сказал(1, "В", _, незнакомы).                                     
%Шипосливы украл А.
сказал(2, "В", Гипотеза, _) :- входит_в( подозреваемый("А", виновен, _),Гипотеза).
%Третье высказывание Б – правда.
сказал(3, "В", Гипотеза, Знакомство) :- сказал(3, "Б", Гипотеза, Знакомство).


%проверяем высказывание
проверить_высказывание(Подозреваемый, Номер, Гипотеза, Знакомство) :- входит_в(подозреваемый(Подозреваемый, _, Правдивость_высказываний),Гипотеза),
...
проверка_высказываний_подозреваемого(Подозреваемый, Гипотеза, Знакомство) :- проверить_высказывание(Подозреваемый, 1, Гипотеза, Знакомство),
...
содержит_количество(Первый, [Первый|Хвост_списка], Количество):- содержит_количество(Первый, Хвост_списка, Количество_1),
                                                                 Количество = Количество_1 + 1.
содержит_количество(Признак, [Первый|Хвост_списка], Количество):- содержит_количество(Признак, Хвост_списка, Количество_1),
                                                                  not(Признак = Первый),
...
%генерация гипотезы
генерация_и_проверка_гипотезы(Гипотеза) :- возможная_вина(Вина_А), возможная_вина(Вина_Б), возможная_вина(Вина_В),
        Список_виновных = [Вина_А, Вина_Б, Вина_В], содержит_количество(виновен, Список_виновных, 1),
        возможный_список_правдивости_высказываний(Правдивость_А), содержит_количество(ложно, Правдивость_А, Количество_А), Количество_А >= 2,
        возможный_список_правдивости_высказываний(Правдивость_Б), содержит_количество(ложно, Правдивость_Б, Количество_Б), Количество_Б >= 2,
        возможный_список_правдивости_высказываний(Правдивость_В), содержит_количество(ложно, Правдивость_В, Количество_В), Количество_В >= 2,
        возможное_знакомство_А_и_В(Знакомство),

        Гипотеза = [подозреваемый("А", Вина_А, Правдивость_А),
...
отсев_дублей([Первый|Хвост_списка], Новый_список) :- отсев_дублей(Хвост_списка, Новый_список_1),
...
%находим виновного
поиск_решения(Пирог_украл) :- findall(Гипотеза, генерация_и_проверка_гипотезы(Гипотеза), Список_гипотез),
