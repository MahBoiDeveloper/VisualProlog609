...
% вспомогательные-предикаты
    ...
    найти_по_номеру(Номер, X, [ X | _ ])
    :-
        Номер - 1 = 0
    .
    найти_по_номеру(Номер, X, [ _ | Хвост ])
    :-
        Номер_1 = Номер - 1,
...
% возможные варианты
    ...
    возможный_список_истинности(Количество, Список)
    :-
        not(Количество = 0),
    ...
    :-
        ...
        not(Список_1 = Список_2), not(Список_1 = Список_3), 
        not(Список_2 = Список_3),
...
% Жюль
    ...
    записал(жуль, 1, Список_блюд)
    :-
        входит_в(блюдо(сначала_выпил, виски), Список_блюд)
    .
    % затем съел утку с апельсинами
    записал(жуль, 2, Список_блюд)
    :-
        входит_в(блюдо(затем_съел, утка_с_апельсинами), Список_блюд)
    .
    % Наконец, он выпил кофе.
    записал(жуль, 3, Список_блюд)
    :-
        входит_в(блюдо(запил, кофе), Список_блюд)
    .
% Жак
    % Он выпил аперитив
    записал(жак, 1, Список_блюд)
    :-
        входит_в(блюдо(сначала_выпил, аперитив), Список_блюд)
    .
    % а затем съел пирог
    записал(жак, 2, Список_блюд)
    :-
        входит_в(блюдо(затем_съел, пирог), Список_блюд)
    .
    % и грушу "прекрасная Елена".
    записал(жак, 3, Список_блюд)
    :-
        входит_в(блюдо(десерт, груша), Список_блюд)
    .
% Джим
    % Сначала он выпил виски
    записал(джим, 1, Список_блюд)
    :-
        входит_в(блюдо(сначала_выпил, виски), Список_блюд)
    .
    % затем съел пирог
    записал(джим, 2, Список_блюд)
    :-
        входит_в(блюдо(затем_съел, пирог), Список_блюд)
    .
    % и клубничный шербет.
    записал(джим, 3, Список_блюд)
    :-
        входит_в(блюдо(десерт, клубничный_щербет), Список_блюд)
    .
    % А закончил он завтрак чашкой кофе.
    записал(джим, 4, Список_блюд)
    :-
        входит_в(блюдо(запил, кофе), Список_блюд)
    .
...
% и список истинности этих записей
    генерация_гипотезы(Завтрак)
    :-
        возможный_список_истинности(3, Истинность_1),
