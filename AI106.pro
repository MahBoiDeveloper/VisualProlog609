...
CLAUSES
    ...
    инверсия([ Имя1 | Хвост ], Имя)
    :-
        Хвост = [ Имя2 | _ ],
        один_из_участников(Имя),
        not(один_из(Имя, Имя1)),
        not(один_из(Имя, Имя2))
    .
    ...
    проверка(Персонаж, Номер, Ответ, Процесс)
    :-
        Персонаж = персонаж(_, рыцарь),
        сказал(Персонаж, Номер, Ответ, Процесс)
    .
    проверка(Персонаж, Номер, Ответ, Процесс)
    :-
        Персонаж = персонаж(_, лжец),
        not(сказал(Персонаж, Номер, Ответ, Процесс))
    .
    проверка(Персонаж, _, _, _)
    :-
        Персонаж = персонаж(_, шпион)
    .

    исключить_подозреваемого(_, [], []).
    исключить_подозреваемого(Имя, [ персонаж(Имя, _) | InT ], OutList)
    :- 
        исключить_подозреваемого(Имя, InT, OutList)
    .
    исключить_подозреваемого(Имя, [ InH | InT ], [ InH | OutT ])
    :-
        not(InH = персонаж(Имя, _)),
        исключить_подозреваемого(Имя, InT, OutT)
    .

% Генерация Гипотезы
    гипотеза(Процесс)
    :-
        один_из_типов(ТипА), один_из_типов(ТипБ), один_из_типов(ТипВ),
    ...
    сказал(персонаж(а, _), 1, "да", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, "да", _, _, _), 
        один_из(персонаж(а, шпион), Персонажи)
    .
    сказал(персонаж(а, _), 1, "нет", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, "нет", _, _, _), 
        not(один_из(персонаж(а, шпион), Персонажи))
    .
    сказал(персонаж(б, _), 1, "да", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, _, "да", _, _), 
        А = персонаж(а, _),
        один_из(А, Персонажи),
        сказал (А, 1, "да", Гипотеза)
    .
    сказал(персонаж(б, _), 1, "нет", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, _, "нет", _, _), 
        А = персонаж(а, _),
        один_из(А, Персонажи),
        not(сказал (А, 1, "нет", Гипотеза))
    .
    сказал(Персонаж, 2, "да", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, _, _, _, "да"),  
        один_из(Сосед, Персонажи),
        not(Сосед = Персонаж),
        Сосед = персонаж(_, шпион)
    .
    сказал(Персонаж, 2, "нет", Гипотеза)
    :-
        Гипотеза = процесс(Персонажи, _, _, _, "нет"), 
        один_из(Сосед, Персонажи),
        not(Сосед = Персонаж),
        Сосед = персонаж(_, Тип),
        not(Тип = "шпион")
    .
          
    проверка_гипотезы(Процесс)
    :-
        гипотеза(Процесс),
        Процесс = процесс(Персонажи, Ответ1, Ответ2, _, _),
        ПерсонажА = персонаж(а, _),
        один_из(ПерсонажА, Персонажи),
    ...
    :-
        ...
        (
            Имя,
            один_из(персонаж(Имя, шпион), Персонажи), 
            Шпионы
        )
    .
              
    найти_не_шпиона(Имя, Ответ1, Ответ2)
    :-
        findall
        (
            Шпионы,
            поиск_шпионов(Шпионы, "да", "да"), 
            ВсеШпионы
        ),
    ...
    :-
        ...
        Подозреваемый = персонаж(ДалОтвет3, _),
        один_из(Подозреваемый, Персонажи2),
        проверка(Подозреваемый, 2, Ответ3, Процесс2)
    .

    поиск_шпионов2(Шпионы, ДалОтвет3, Ответ3)
    :-
        проверка_соседей(Процесс),
    ...
    найти_шпиона(ИмяШпиона)
    :-
        findall
        (
            Шпионы,
            поиск_шпионов2(Шпионы, а, "да"), 
            ВсеШпионы
        ),
        решение_без_повторов(ВсеШпионы, [], Шпионы), 
        количество_элементов(Шпионы,КоличествоРешений), 
        КоличествоРешений = 1,
        Шпионы = [ [ ИмяШпиона ] | _ ],
