...
CLAUSES
	...
	один_из(Ученик,[_|Хвост_Списка_Учеников]):-один_из(Ученик,Хвост_Списка_Учеников).
	...
	количество([], 0).
 	количество([_Первый|Хвост_количества], Количество):-
    	...
    	количество_истинных(Имя, Подозреваемые, Количество):-
     		findall(Номер, говорит(Имя, Номер, Подозреваемые), НомераПравды), 
	...
	генерация_гипотезы(Подозреваемые):-    
		...
		возможный_разговор(Разговаривают),
                Подозреваемые=[ученик(ангелика, ВинаА), ученик(бернд,ВинаБ), 
                  ученик(вольфганг,ВинаB), ученик(мануэла,ВинаМ),
                  разговаривают(бернд, мануэла, Разговаривают)],
                findall(Имя, один_из(ученик(Имя, разбил), Подозреваемые), Виновные),
